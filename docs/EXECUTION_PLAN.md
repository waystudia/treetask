...
- [ ] Полный authenticated CRUD с membership-проверками для всех сущностей:
  ...
  через offline push/pull; удаление/редактирование файлов, membership UI и
  notifications CRUD ещё требуются.
...
- Unit: 21 тест проходит (domain 10, collab 2, web 9).
- Playwright: 20 сценариев проходят, 16 профильных дублей осознанно пропущены;
  проверены desktop, tablet и phone, включая проекты и файлы offline-first.
...
- `pnpm test:db` локально пока не запускался: Supabase CLI поднимает pgTAP через
  Docker, которого нет на текущем Mac.
- Production build проходит. Browser QA свежего build выполнена при 1440×900,
  768×1024 и 390×844: overflow и console errors не обнаружены.
